class LuxPowerDistributionCard extends HTMLElement {
  ...
  setConfig(config) {
      ...
      if (!config.backup_power || !config.backup_power.entity) {
        throw new Error('You need to define an entity for the backup power (EPS).');
      }
  ...
  _update() {
    if (this._hass.states[this.config['battery_soc'].entity] == undefined ||
  ...
  _updateStates() {
    ...
    this.nodes['battery_soc'].innerHTML = this._formatValue(batterySocValue, '%');
    ...
    this.nodes['battery_flow'].innerHTML = this._formatValue(batteryFlowValue, 'W');
    const batteryFlowDirection = 'arrow-cell arrows-' + ((batteryFlowValue > 0) ? 'left' : ((batteryFlowValue < 0) ? 'right' : 'none'));
    const batteryFlowDescription = batteryFlowValue > 0 ? 'Battery Charging' : (batteryFlowValue < 0 ? 'Battery Discharging' : '');
    ...
    // Home Consumption
    const homeConsumptionValue = this._getConfigEntityStateValue('home_consumption');
    this._setCssVariable(this.nodes.content, 'home-consumption', '--W');
    this.nodes['home_consumption'].innerHTML = this._formatValue(homeConsumptionValue, 'W');
    ...
    const gridFlowDirection = 'arrow-cell arrows-' + ((gridFlowValue < 0) ? 'left' : ((gridFlowValue > 0) ? 'right' : 'none'));
    
    // Not-required Entities
    // Battery Voltage
    const batteryVoltageValue = this._getConfigEntityStateValue('battery_voltage');
    this._setCssVariable(this.nodes.content, 'battery-voltage', '--Vdc');
    this.nodes['battery_voltage'].innerHTML = this._formatValue(batteryVoltageValue, 'Vdc', false);
    ...
    const pvPowerDirection = 'arrow-cell arrows-' + ((pvPowerValue > 0) ? 'down' :'none');
    ...
    this.nodes['backup_power'].innerHTML = this._formatValue(epsPowerValue, 'W');
    const homeConsumptionDirection = 'arrow-cell arrows-' + ((homeConsumptionValue > 0 || epsPowerValue > 0) ? 'down' : 'none');
    ...
    this.nodes['grid_voltage'].innerHTML = this._formatValue(gridVoltageValue, 'Vac', false);
  ...
  _createCard() {
      ...
      <ha-card>
            ...
            <div class="diagram-grid">
                ...
                <div class="cell arrow-cell arrows-down">
                    <div class="arrow-1"><img src="https://img.icons8.com/color/256/chevron-left.png"></div>
                    <div class="arrow-2"><img src="https://img.icons8.com/color/256/chevron-left.png"></div>
                    <div class="arrow-3"><img src="https://img.icons8.com/color/256/chevron-left.png"></div>
                    <div class="arrow-4"><img src="https://img.icons8.com/color/256/chevron-left.png"></div>
                </div>
                <div class="cell"></div>
                <div class="cell"></div>
                <div class="cell"></div>
    ...
    this.nodes = {
      ...
      battery_flow_description: content.querySelector('.battery-flow-description'),
